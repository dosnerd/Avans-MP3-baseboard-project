 ...
 * fails, means that the SPI communication doesn't work, the output of the VS1033 or the VS1033 doesn't work
 * at all. Don't run this test while communicating with the VS1033.
 *
...
public class SinusTest {
     ...
     */
    public SinusTest() {
        IO.UI.println("Initialize VS1033");
        ...
        try {
            ...
            //send commands, wait between commands so the VS1033 can run the command
            IO.UI.println("Init");
            SCI.write(init);
            Thread.sleep(1);
            SCI.write(vol);
    ...
    /**
     * Send the command to the VS1033 to run the sinus test. The VS1033 give on its output a sinus signal.
     */
    ...
    /**
     * End the test by stopping the VS1033 with sending the sinus and close the interfaces.
     */
    public void endTest() {
        try {
            //create commando and send it
